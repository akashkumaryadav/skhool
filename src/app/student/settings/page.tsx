...
import { 
  UserCircleIcon, BellAlertIcon, PaintBrushIcon, 
  ShieldCheckIcon, Cog6ToothIcon, PencilIcon 
} from '../../constants';
...
const ToggleSwitch = ({ id, label, enabled, setEnabled, description }: ToggleSwitchProps): React.ReactNode => {
  return (
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto pb-12">
      <div className="text-center md:text-left">
        ...
        <p className="text-gray-600 mt-1">Manage your Skhool account preferences.</p>
      </div>

      {/* Profile Section */}
      <DashboardCard title="My Profile" icon={<UserCircleIcon className="w-7 h-7 text-skhool-blue-600" />}>
        <div className="flex items-center space-x-4 mb-6 p-4 bg-gray-50 rounded-lg">
          <img 
            src={mockStudentUser.avatarUrl} 
            alt={`${mockStudentUser.name}'s avatar`}
            className="w-16 h-16 rounded-full object-cover shadow-md"
          />
          <div>
            <h3 className="text-xl font-semibold text-gray-800">{mockStudentUser.name}</h3>
            <p className="text-sm text-gray-600">{mockStudentEmail}</p>
            <p className="text-xs text-skhool-orange-500 font-medium mt-0.5">
              Role: {mockStudentUser.role} | Class: {mockStudentClass}-{mockStudentSection}
            </p>
          </div>
        </div>
        <button 
