...
export function middleware(req: NextRequest) {
    ...
    let user;
    try {
        user = userToken ? JSON.parse(Buffer.from(userToken, 'base64').toString('utf-8')) : null;
    }
    catch (error) {
...
// 'ROLE_teacher'
    ...
    if (currentRole === 'teacher' && req.nextUrl.pathname.startsWith(baseStudentRoute)) {
    ...
    if (currentRole === 'student' && req.nextUrl.pathname.startsWith(baseTeacherRoute)) {
    ...
    if (currentRole === 'admin' && req.nextUrl.pathname.startsWith(baseStudentRoute)) {
    ...
    if (currentRole === 'admin' && req.nextUrl.pathname.startsWith(baseTeacherRoute)) {
